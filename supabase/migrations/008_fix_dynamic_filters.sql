-- =============================================================================
-- Migration 008: Fix get_dynamic_filter_options — table alias scope bug
--
-- The disease_states and disease_types queries use UNION ALL subqueries
-- wrapped in a parent SELECT. The parent SELECT referenced q.id, but the
-- 'q' alias is only available INSIDE the UNION subqueries. The parent query
-- only sees the 'sub' alias, so it must use COUNT(DISTINCT id) not COUNT(DISTINCT q.id).
--
...
BEGIN
    ...
    -- FIX: Use COUNT(DISTINCT id) not COUNT(DISTINCT q.id) — q is not in scope in outer query
    -- ========================================
    v_where := v_base || c_topic || c_disease_stage || c_disease_type || c_treatment_line
        || c_treatment || c_biomarker || c_trial || c_treatment_elig || c_age_group || c_fitness || c_organ || c_advanced;
    EXECUTE format(
        'SELECT COALESCE(json_agg(json_build_object(''value'', val, ''count'', cnt) ORDER BY cnt DESC), ''[]''::JSON) '
        || 'FROM (SELECT ds as val, COUNT(DISTINCT id) as cnt FROM ('
    ...
    EXECUTE format(
        'SELECT COALESCE(json_agg(json_build_object(''value'', val, ''count'', cnt) ORDER BY cnt DESC), ''[]''::JSON) '
        || 'FROM (SELECT dt as val, COUNT(DISTINCT id) as cnt FROM ('
