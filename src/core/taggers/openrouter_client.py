...
class OpenRouterClient:
    ...
    async def generate(
        ...
        Returns:
            Dict with 'content', 'usage', 'model', and 'cost' keys
        """
        if not self.api_key:
        ...
        try:
            logger.debug(f"Calling {model_config.display_name} with {len(messages)} messages")
            ...
            logger.error(f"Error calling {model_config.display_name}: {e}")
            raise
